...
func _draw() -> void:
	...
	# Track
	draw_arc(center, RADIUS, 0, TAU, 32, TRACK_COLOR, 3.0)
	# Arc progress
	if GameState.is_night:
		draw_arc(center, RADIUS, -PI * 0.5, -PI * 0.5 + TAU * 0.3, 24, MOON_COLOR, 3.0)
		# Moon symbol
		draw_circle(center + Vector2(0, -RADIUS * 0.45), 4, MOON_COLOR)
	else:
		var t   := GameState.time_of_day
		var end := -PI * 0.5 + TAU * t
		draw_arc(center, RADIUS, -PI * 0.5, end, 32, SUN_COLOR, 3.0)
		# Sun dot
		var sun_pos := center + Vector2(cos(-PI * 0.5 + TAU * t), sin(-PI * 0.5 + TAU * t)) * RADIUS
		draw_circle(sun_pos, 3, SUN_COLOR)
